...
class CSVDataset:
    ...
    def __init__(self, dat_file, cnf_file, mode=0, img_mode=0, arch=None, transforms=None, stripped=None):
        ...
        def is_float_nan(x):
            return x is np.nan or isinstance(x, float) and np.isnan(x)
        ### replacement done
        contains_nan = df.applymap(is_float_nan).values.any()

        if contains_nan:
            print("yes")
            raise ValueError("NaN values are found in the dataframe.")
        else:
            # print("no")
            pass
        # print(df)
        ...
        for key in df.columns:
            if key in label_names:
                print(key)
        # raise ValueError("Check the labels in the csv file")
        ###############################
